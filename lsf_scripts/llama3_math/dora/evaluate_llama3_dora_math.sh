...
#BSUB -q cal_h8
#BSUB -P ama02
#BSUB -J evaluate_llama3_dora_math
#BSUB -n 1
#BSUB -R "span[ptile=1]"
#BSUB -gpu "num=8:j_exclusive=yes"
#BSUB -rn
#BSUB -o lsf_logs/evaluate_llama3/math/dora_eval.%J.log
#BUSB -o lsf_logs/evaluate_llama3/math/dora_eval.%J.err
...
PEFT_NAME_OR_PATH=hf_models/${MODEL_NAME}/math/dora/seed${SEED}_lr${LR}
    ...
    if [ $NAME = "AQuA" ]; then
        singularity run --nv uv-cuda12.sif \
            uv run python experiments/llama3_math/evaluate_math.py \
                ...
                --output-json-path lsf_logs/evaluate_llama3/math/dora_eval_${NAME}_seed${SEED}_lr${LR}.json \
                --is-letter-answer \
                --batch-size 128
    else
        singularity run --nv uv-cuda12.sif \
